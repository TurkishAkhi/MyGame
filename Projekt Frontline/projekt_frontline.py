...
# -----------------------------------------
#  PROJEKT FRONTLINE  -  v4.0  (BUGFIXED)
#  6 Zonen, Tanks, Drohnen, Jets,
#  Helikopter, Jetpack-Soldaten,
#  Schrotflinte, Raketenwerfer, Stinger, Messer
# -----------------------------------------
...
class SoundManager:
    def __init__(self):
        ...
        try:
            ...
            self.enabled = False
            # Dummy-Sounds damit SFX.play() nicht crasht
            class _Dummy:
                def play(self): pass
            for attr in ["shoot_pistol","shoot_rifle","shoot_sniper","shoot_shotgun",
                         "shoot_rocket","shoot_stinger","knife_slash","explosion",
                         "big_explosion","hit_player","hit_enemy","jump",
                         "grenade_throw","zone_clear","boss_roar","tank_shot"]:
...
class Particle:
    ...
    def update(self):
        ...
        self.vy+=self.gravity; self.vx*=PARTICLE_FRICTION; self.life-=1
...
class ParticleSystem:
    ...
    def spawn_muzzle_flash(self, x, y, direction):
        for _ in range(8):
            ...
            self.particles.append(Particle(x,y,math.cos(a)*spd*direction,
                                           math.sin(a)*spd,random.randint(4,10),
                                           col,random.randint(2,5),0))
    ...
    def spawn_blood(self, x, y, count=12):
        for _ in range(count):
            ...
            self.particles.append(Particle(x,y,math.cos(a)*spd,
                                           math.sin(a)*spd-2,random.randint(15,30),
    ...
    def spawn_explosion(self, x, y, scale=1.0):
        ...
        for _ in range(count):
            ...
            self.particles.append(Particle(x,y,math.cos(a)*spd,math.sin(a)*spd-3,
                                           random.randint(20,45),col,
                                           random.randint(4,max(4,int(8*scale))),0.25))
        for _ in range(int(20*scale)):
            a=random.uniform(0,2*math.pi); spd=random.uniform(0.5,3)
            self.particles.append(Particle(x,y,math.cos(a)*spd,math.sin(a)*spd-4,
                                           random.randint(35,60),(80,80,80),
                                           random.randint(6,12),-0.05))

    def spawn_dust(self, x, y):
        for _ in range(5):
            self.particles.append(Particle(x,y,random.uniform(-2,2),
                                           random.uniform(-3,-1),random.randint(8,18),
                                           (120,100,70),random.randint(2,4)))

    def spawn_shell(self, x, y, direction):
        self.particles.append(Particle(x,y,-direction*random.uniform(2,5),
                                       random.uniform(-4,-1),random.randint(20,35),
                                       (200,180,50),3))
    ...
    def spawn_sparks(self, x, y, count=10):
        for _ in range(count):
            ...
            self.particles.append(Particle(x,y,math.cos(a)*spd,math.sin(a)*spd,
                                           random.randint(5,15),col,2,0.3))

    def spawn_smoke(self, x, y):
        self.particles.append(Particle(x,y,random.uniform(-1,1),
                                       random.uniform(-2,-0.5),random.randint(20,40),
                                       (100,100,100),random.randint(4,8),-0.02))
...
class Camera:
    ...
    def in_view(self, wx, margin=300):
        return -margin < wx-self.x < WIDTH+margin
...
class Weapon:
    ...
    # BUG FIX: Messer braucht kein self.facing – direction wird übergeben
    def shoot_toward(self, ox, oy, tx, ty, facing=1):
        if not self.can_shoot(): return []
        self.last_shot=pygame.time.get_ticks()
        if self.ammo is not None: self.ammo-=1
        if self.sound: SFX.play(self.sound)

        # Messer: gibt Melee-Tuple zurück, kein Projektil
        if self.is_knife:
            return [("melee", ox, oy, facing)]
...
class Grenade:
    ...
    def __init__(self,x,y,direction):
        ...
        self.vx=direction*10; self.vy=-14
    ...
    def update(self,enemies):
        ...
        for plat in current_platforms:
            gr=pygame.Rect(self.x-self.RADIUS,self.y-self.RADIUS,
                           self.RADIUS*2,self.RADIUS*2)
...
class Player:
    ...
    def _fire(self,bullets,rockets,cam_x):
        ...
        # BUG FIX: facing wird übergeben
        results=self.weapon.shoot_toward(ox,oy,tx,ty,self.facing)
        for r in results:
    ...
    def get_knife_hit_rect(self):
        return pygame.Rect(self.rect.centerx+self.facing*5,
                           self.rect.centery-10,50,30)
    ...
    def draw(self,surf,cam_x=0):
        ...
        if self.weapon==MESSER and self.knife_anim>0:
            ...
            pygame.draw.line(surf,(200,200,200),(arm_end_x,arm_end_y),
                             (arm_end_x+f*20,arm_end_y-5),3)
...
class Enemy:
    ...
    def _try_jump(self,target_x):
        ...
        dx=target_x-self.x
        for plat in current_platforms:
            # BUG FIX: plat.height statt plat.h
...
class JetpackSoldier:
    W=28; H=48; MAX_HP=80
    is_air=True  # BUG FIX: Klassenattribut statt Property
    ...
    def update(self,player,bullets):
        ...
        self.vy+=(target_vy-self.vy)*0.1; self.y+=self.vy
        self.strafe_timer+=1
...
class Tank:
    ...
    def __init__(self,x,y):
        # BUG FIX: y korrekt berechnen (auf Boden stellen)
    ...
    def update(self,player,bullets):
        ...
        if now-self.last_shot>=self.SHOOT_RATE and abs(dx)<700:
            ...
            tx=player.rect.centerx; ty=player.rect.centery
            d2=math.hypot(tx-ox,ty-oy) or 1; spd=14
...
class Drone:
    ...
    def __init__(self,x):
        ...
        self.hp=self.MAX_HP; self.alive=True; self.facing=-1
        self.last_shot=0; self.bob_offset=random.uniform(0,2*math.pi)
...
class Boss:
    ...
    def update(self,player,bullets,enemy_rockets):
        ...
        self.x=max(0,min(WORLD_WIDTH-self.W,self.x))
        now=pygame.time.get_ticks(); rate=self.SHOOT_RATE//self.phase
        if now-self.last_shot>=rate and abs(dx)<800:
            self.last_shot=now; spread=self.BURST_SIZE+self.phase
            SFX.play(SFX.shoot_rifle)
            ox=self.rect.centerx; oy=self.rect.centery
            tx=player.rect.centerx; ty=player.rect.centery
...
def draw_hud(surf,player,zone_num,zone_name,enemies_left,air_left):
    ...
    pygame.draw.rect(surf,hc,(14,52,int(200*hr),14))
    pygame.draw.rect(surf,GRAY,(14,52,200,14),1)
    ...
    surf.blit(font_tiny.render("[5] Rakete→Tanks  |  [6] Stinger→Luft",
                               True,(200,200,100)),(WIDTH//2-160,HEIGHT-36))
...
def show_main_menu():
    ...
    while True:
        ...
        hint=tf.render("WASD: Bewegen  |  SPACE: Springen  |  Klick: Schiessen  |  G: Granate  |  1-7: Waffe",
                       True,(70,70,90))
...
def show_controls():
    ...
    ctrls=[
        ...
        ("3","Scharfschuetze  (55 DMG)"),("4","Schrotflinte  (8x18 DMG)"),
        ("5","Raketenwerfer  (120 DMG) → Tanks!"),
        ("6","Stinger AA  (150 DMG) → Luft!"),
        ("7","Messer  (35 DMG, Nahkampf)"),("ESC","Menue"),
    ]
    tick=0; kf=pygame.font.SysFont("consolas",16,bold=True)
    vf=pygame.font.SysFont("consolas",15); tf=pygame.font.SysFont("consolas",36,bold=True)
    while True:
        tick+=1; mx,my=pygame.mouse.get_pos()
        for event in pygame.event.get():
            if event.type==pygame.QUIT: pygame.quit(); sys.exit()
...
def show_name_input(score,zone):
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type==pygame.KEYDOWN:
                if event.key==pygame.K_RETURN and name.strip():
                    save_highscore(name.strip()[:12],score,zone); return
                elif event.key==pygame.K_BACKSPACE: name=name[:-1]
                elif len(name)<12 and event.unicode.isprintable(): name+=event.unicode
            if event.type==pygame.MOUSEBUTTONDOWN and event.button==1:
                if confirm.clicked((mx,my)) and name.strip():
                    save_highscore(name.strip()[:12],score,zone); return
        draw_menu_bg(screen,tick)
        t=tf.render("HIGHSCORE EINTRAGEN",True,YELLOW)
        screen.blit(t,(WIDTH//2-t.get_width()//2,HEIGHT//2-200))
        sc=inf.render(f"Score: {score}   |   Zone {zone}",True,WHITE)
        screen.blit(sc,(WIDTH//2-sc.get_width()//2,HEIGHT//2-140))
        ...
        it=inf.render(name+cursor,True,WHITE)
        screen.blit(it,(ir.centerx-it.get_width()//2,ir.centery-it.get_height()//2))
...
def show_gameover(score,zone):
    ...
    while True:
        ...
        t2=f2.render(f"Score: {score}   |   Zone {zone} erreicht",True,GRAY)
...
def play_zone(zone_num,player):
    ...
    while True:
        ...
        # BUG FIX: all_air NACH dem Update bauen damit Stinger korrekt zielt
        # Feinde updaten
        for e in enemies:
        ...
        # Alle Luftziele (für Stinger-Homing)
        ...
        # BUG FIX: Feind-Granaten treffen auch den Spieler
        ...
        if all_dead:
            if is_boss_zone:
                if boss is None or not boss.alive:
                    SFX.play(SFX.zone_clear); return "win"
            else:
